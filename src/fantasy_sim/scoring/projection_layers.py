...
from fantasy_sim.scoring.residual_calibration import ResidualCalibrationStats
...
class ResidualCalibrationAdjusterProtocol(Protocol[ProjectionLayerRowT]):
    def adjust_week(
        self,
        projections: list[ProjectionLayerRowT],
        *,
        season: int,
        week: int,
    ) -> tuple[list[ProjectionLayerRowT], ResidualCalibrationStats]: ...
...
def apply_projection_layers(
    ...
    *,
    ...
    residual_calibrator: ResidualCalibrationAdjusterProtocol[
        ProjectionLayerRowT
    ] | None = None,
    ...
    if dynamic_blender is not None:
        ...
        if market_history_adjuster is not None:
            rows, _market_stats = market_history_adjuster.adjust_week(
                rows,
                season=season,
                week=week,
            )
    ...
    if residual_calibrator is not None:
        rows, _residual_stats = residual_calibrator.adjust_week(
            rows,
            season=season,
            week=week,
        )
