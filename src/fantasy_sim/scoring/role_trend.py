...
class RoleTrendProjectionAdjuster:
    ...
    @staticmethod
    def _mark_neutral(row: dict) -> dict:
    ...
    def adjust_week(
        ...
        *,
        ...
        for projection in projections:
            ...
            recent_opportunity = self._opportunity(recent, position)
            baseline_opportunity = self._opportunity(baseline, position)
            if baseline_opportunity <= 0:
                baseline_opportunity = recent_opportunity
            if baseline_opportunity <= 0:
                output.append(self._mark_neutral(row))
                continue

            ratio = recent_opportunity / baseline_opportunity
