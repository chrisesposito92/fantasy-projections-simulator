...
- **`~/.fantasy-sim/market-history/processed/player_markets_*_close_core8.parquet`** — Existing schema (18 columns including `bookmaker_count`, `line`, `line_stddev`, `over_price`, `under_price`, `implied_prob`) handles multi-line alt-line markets without changes. New snapshots get distinct `snapshot_label` values (`prior_core8`, `close_alt6`, `prior_alt6`) per the revised D-06 (Cycle 1 HIGH-3 reconciliation; `prior_*` honestly describes the API `previous_timestamp` semantic, replacing the misleading `open_*` prefix from the original draft).
...
- **Real Tuesday-line-release marker follow-up** — Per revised D-02 (Cycle 1 HIGH-3 reconciliation) the `prior_*` snapshots use the API `previous_timestamp` relative to the existing gameday-noon UTC events crawl, NOT a true Tuesday 12pm ET line-release marker. If Phase 4 engine integration finds the API previous-timestamp too noisy, a follow-up phase may extend `events_inventory.py` with a separate Tuesday-noon crawl populating `events_inventory_tuesday_*.parquet`.
