...
- **PFF Data Scraper**: Complete — 619 tests. Standalone script (`scripts/scrape_pff.py`) extracts game-level player data from PFF's premium REST API (21 facets per game). Supports NFL and NCAA via `--league` flag (`nfl` default, `ncaa`). Stores raw JSON as immutable archive at `~/.fantasy-sim/pff/raw/<league>/`, processes into per-facet-per-season parquet at `~/.fantasy-sim/pff/processed/<league>/`. NCAA weeks are 0-16 (vs NFL 1-18 + postseason). Cookie-based auth, resume via progress.json, exponential backoff on rate limits. See `docs/pff-setup.md` for setup.
